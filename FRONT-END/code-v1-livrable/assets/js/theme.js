...
var totalSalesInit = function totalSalesInit() {
  ...
  if ($echartsLineTotalSales) {
    ...
    var fetchChartData = function () {
      return fetch('http://localhost:8000/monthly_reviews_chart')
        .then(response => response.json())
        .then(data => data);
    };

    // Fonction pour initialiser le graphique avec les données récupérées
      ...
      var options = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            var date = params[0].name; // Date du mois
            var value = params[0].value; // Nombre d'avis
            return `${date}: ${value} avis`;
          }
        },
        xAxis: {
          type: 'category',
          data: data.months, // Mois obtenus de l'API
          axisLabel: {
            color: utils.getGrays()['400']
          },
          axisLine: {
        ...
        series: [{
          type: 'line',
          data: data.review_counts, // Nombre d'avis obtenus de l'API
          lineStyle: {
            color: utils.getColors().primary
          },
          itemStyle: {
